...
def compare_model_pissa(model_a: "torch.nn.Module", model_b: "torch.nn.Module", adapter: str = "default") -> None:
    ...
    rtol, atol = 1e-3, 1e-4
    # The top-r/rest split PiSSA truncates to is only a *unique* best rank-r approximation
    # while sigma_r is strictly greater than sigma_{r+1} (Eckart-Young). If that gap is too
    # small, different SVD backends can legitimately select a different set of "top r"
    # directions (not just a rotation within the same subspace), so B @ A would then differ
    # for a real mathematical reason rather than a bug. Guard the precondition explicitly
    # instead of assuming the test model's weights always have a comfortable gap. The bar is
    # set relative to `rtol`, the same fp16 noise floor already used for the deltas below, so
    # a real (if modest) spectral gap isn't mistaken for a near-degeneracy.
    min_gap_ratio = 1 + rtol
    for name, module_a in modules_a.items():
        module_b = modules_b[name]
        r = module_a.r[adapter]
        ...
        original_weight = module_a.base_layer.weight.detach().float().cpu() + delta_a.float()
        singular_values = torch.linalg.svdvals(original_weight)
        sigma_r, sigma_r1 = singular_values[r - 1].item(), singular_values[r].item()
        assert sigma_r > sigma_r1 * min_gap_ratio, (
            f"singular value gap at rank {r} for {name} is too small "
            f"(sigma_{r}={sigma_r:.4g}, sigma_{r + 1}={sigma_r1:.4g}); the top-{r} subspace "
            "is not well-defined for this weight, so comparing B @ A across backends is unreliable here"
        )
